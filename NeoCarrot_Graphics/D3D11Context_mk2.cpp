...
void D3D11Context_mk2::CreateSwapChain(HWND hWnd, int width, int height)
{
    ...
    // 스왑 체인 생성
    dxgiFactory->CreateSwapChain(_devices->Device(), &swapChain, _swapChain.GetAddressOf());
...
void D3D11Context_mk2::OnResize(int width, int height)
{
    ...
    // 랜더 타겟 뷰 생성
    assert(backBuffer != 0);
    _devices->Device()->CreateRenderTargetView(backBuffer, 0, _resourceView->renderTargetView.GetAddressOf());
    ...
    _devices->Device()->CreateDepthStencilView(_depthStencilBuffer.Get(), 0, &_resourceView->depthStencilView); // 깊이/스텔실 뷰 반환
    ...
    _devices->ImmediateContext()
        ->OMSetRenderTargets(1, _resourceView->renderTargetView.GetAddressOf(), _resourceView->depthStencilView.Get());
    assert(_resourceView->renderTargetView);
